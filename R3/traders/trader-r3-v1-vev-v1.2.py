"""
R3 Stage-2a v1.2 trader: VEV ecosystem (VELVETFRUIT_EXTRACT + 6 vouchers).

Diff vs v1.1: passive-first execution. Voucher orders post limit AT our
side (buy at best_bid, sell at best_ask) instead of crossing the spread;
they escalate to marketable only when the signal is strong (|z| >=
aggressive_z_threshold). VEV hedge orders post passive too; they escalate
when the unhedged net delta breaches the hedge band OR after
hedge_passive_wait_ticks of a pending hedge.

Inherits v1.1's min_hold/cooldown gating on Module A.

Goal: convert ~36/trade slippage into spread capture (or at least
neutralize it) so paper alpha translates to realized PnL.
...
class Trader:
    def run(self, state: TradingState):
        ...
        for K, tgt in combined_voucher_targets.items():
            ...
            orders = emit_passive_or_marketable(sym, depth, cur, tgt, escalate=z_strong)
            # update pending_wait
            if cur == tgt:
                pending_wait[sym] = 0
            else:
        ...
        vev_orders = emit_passive_or_marketable(
            PRODUCT_VEV, vev_depth, cur_vev, vev_combined_target,
