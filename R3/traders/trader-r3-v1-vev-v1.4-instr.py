"""
R3 Stage-2a v1.4-instr trader: instrumented copy of v1.4 for diagnostics.

Identical strategy to v1.4 (Module A + B + C with delta hedge); only the
per-tick lambdaLog is enhanced with smile coefs (a, b) and per-strike
passive/aggressive routing flags (pas, agg). Used by Stage-2a-extended
diagnostics to attribute fills without re-running.

Original v1.4 trader: VEV ecosystem (VELVETFRUIT_EXTRACT + 6 vouchers).

Diff vs v1.3: adds per-day current_day routing for accurate TTE in the
3-day backtest. The trader cannot self-detect which historical day it's
running (rust backtester resets timestamp to 0 each day, separate
process per day), so a side-channel is needed. To respect Hard Rule #10
(no `import os`), v1.4 reads an optional `_current_day.txt` file from
the configs directory via pathlib; the rust-backtester wrapper writes it
before each day's invocation. Falls back to JSON `current_day` if the
file is missing.

Smile coef sanity check: hardcoded fallback (a,b,c) = (0.143,-0.002,0.236)
from N4 cell 11 vs N3 cell 23 alternative (0.158,-0.0046,0.232) is moot
in this universe — with 6 active strikes and 100% IV survival, the
fallback never fires (smile_fit_min_strikes=4 always met). Switching
hardcoded values would not change any Stage-2a backtest output.

Inherits all v1.1-v1.3 changes.
...
try:
    ...
    if _cfg_path.is_file():
        with open(_cfg_path, "r") as _f:
            CONFIG = json.load(_f)
    # v1.4: optional per-day override. The backtest wrapper writes the
    # historical day index (0/1/2) here before each rust-backtester run so
    # TTE is correct (day 1 -> 7d, day 2 -> 6d). For live R3 set CONFIG
    # current_day=3 directly via the JSON; this file should not exist on
    # the IMC sandbox (only the .py is uploaded). Pathlib used instead of
    # os.environ to comply with Hard Rule #10 (no `import os`).
    _day_path = _Path(__file__).parent / "configs" / "_current_day.txt"
...
def emit_to_target(symbol: str, depth: OrderDepth, current_pos: int,
    ...
    """v1.0 marketable emit: lifts/hits the opposite touch. Kept for any
    code path that still wants forced-marketable execution."""
...
def emit_passive_or_marketable(symbol: str, depth: OrderDepth, current_pos: int,
    ...
    """v1.2 execution: post limit at our side by default; escalate to
    marketable only when caller signals (escalate=True). Rust matching
    model (runner.rs L612-658) confirms: a buy at best_bid will NOT
    cross any ask (`level.price > order.price` filter), so it sits
    passively and only fills via market-trade flow penetrating our
    queue level. That converts spread cost into spread capture."""
    diff = target_pos - current_pos
    if diff == 0:
        return []
    bb, ba = best_bid_ask(depth)
    if bb is None or ba is None:
        return []
    if escalate:
        # Marketable: lift ask / hit bid, bounded by available volume
        # at the touch level.
        if diff > 0:
        ...
        # Passive: limit at our touch (no immediate cross). Quote for the
        # full target diff; whatever doesn't fill simply expires this
        # tick and we re-quote next tick.
        if diff > 0:
            return [Order(symbol, bb, diff)]
        else:
            return [Order(symbol, ba, diff)]  # diff is negative
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ---- Build orders (v1.2 passive-first) ----------------------------
        # Execution policy:
        #   Vouchers: passive limit at our touch by default. Escalate to
        #     marketable only when |z[K]| >= aggressive_z_threshold (or for
        #     5200/5300, when |z_c| >= aggressive_z_threshold), i.e. when the
        #     signal is strong enough that paying the spread is worth it.
        #   VEV hedge: passive limit by default. Escalate when the *current*
        #     unhedged delta (from filled positions, not target) breaches the
        #     hedge band, OR when this hedge target has been pending for >=
        #     hedge_passive_wait_ticks. Hedge prefers passive but can't drift
        #     unboundedly out-of-band.
        aggressive_z = float(cfg.get("aggressive_z_threshold", 2.5))
        hedge_wait_max = int(cfg.get("hedge_passive_wait_ticks", 1))
        # max_step_size: clip per-tick position diff. Prevents one-shot
        # marketable orders on escalation; small numbers (10) trade for
        # slippage saved at the cost of slower delta convergence.
        ...
        # Per-symbol pending tick counter: ticks since target last differed
        # from current; resets when target == current. Used only by the
        # hedge escalation path; voucher escalation is signal-driven.
        pending_wait: Dict[str, int] = saved.get("pending_wait", {})

        # z_c value for Module B (recomputed for clarity; cheap).
        z_c_val = 0.0
        if len(c_buf) >= 50:
            mu_c = sum(c_buf) / len(c_buf)
            var_c = sum((x - mu_c) ** 2 for x in c_buf) / max(len(c_buf) - 1, 1)
            sd_c = math.sqrt(var_c) if var_c > 0 else 0.0
            if sd_c > 1e-9:
                z_c_val = (c_t - mu_c) / sd_c

        # Diagnostics instrumentation: per-tick passive/aggressive routing.
        ...
        for K, tgt in combined_voucher_targets.items():
            ...
            orders = emit_passive_or_marketable(sym, depth, cur, tgt_step, escalate=z_strong)
            # update pending_wait
            if cur == tgt:
                pending_wait[sym] = 0
            else:
        ...
        # Hedge escalation: based on REALIZED net delta (current voucher
        # positions * delta), not target. If the realized delta is already
        # outside band, we cannot wait passively.
        realized_voucher_delta = 0.0
        for K in active_strikes:
            sym_k = voucher_symbol(K)
            realized_voucher_delta += state.position.get(sym_k, 0) * deltas_per_strike.get(K, 0.0)
        realized_net_delta = realized_voucher_delta + cur_vev
        vev_pending = int(pending_wait.get(PRODUCT_VEV, 0))
        hedge_escalate = (abs(realized_net_delta) > hedge_band) or (vev_pending >= hedge_wait_max)
        vev_tgt_step = _clip_step(cur_vev, vev_combined_target)
        vev_orders = emit_passive_or_marketable(
            PRODUCT_VEV, vev_depth, cur_vev, vev_tgt_step,
            escalate=hedge_escalate,
        )
        if cur_vev == vev_combined_target:
            pending_wait[PRODUCT_VEV] = 0
        else:
            pending_wait[PRODUCT_VEV] = vev_pending + 1
        if vev_orders:
