"""
R3 Stage-2a v1.3 trader: VEV ecosystem (VELVETFRUIT_EXTRACT + 6 vouchers).

Diff vs v1.2: per-tick max_step_size cap on the diff between target and
current position before order emission (vouchers and VEV alike).
Combined with passive-first execution, this prevents big single-shot
marketable lifts/hits when the signal flickers — even on escalation we
step in increments. Helps especially on the VEV hedge, which in v1.2
dominates losses at -390k/day because it crosses spread in 50-lot chunks.

Inherits v1.1 (min_hold/cooldown) and v1.2 (passive-first).
