"""
R3 Phase-3 v1.5-moduleA-only trader: cross-strike smile-residual scalp
(Module A) only by default. Module B (base-IV mean reversion on c_t) is
wired behind a feature flag `enable_module_b` (default False).

Diff vs v1.5-scalp-only:
  - New config key `enable_module_b` (bool, default False). Both the
    Module-B signal-generation block and its contribution to combined
    voucher targets are gated on this flag. Module-B targets default to
    0 for 5200 and 5300 when off; the c_t buffer is not accumulated.
  - Defensive assertion: if `enable_module_b` is False but any
    module_b_targets value is non-zero (e.g. state leaked across a flag
    flip via traderData), the trader raises. Better to crash loudly
    than to silently emit Module-B orders.
  - `aggressive_z_threshold` may be `Infinity` (JSON literal) or the
    string "inf" to disable aggression entirely; with that, the
    comparison `abs(z) >= aggressive_z` is unreachable for any finite z.
  - New config key `smile_fit_strikes` (list[int], default
    `active_strikes`). Decouples the smile-fit input universe from the
    traded universe so we can fit a wider/different set than we trade
    (e.g. fit on 6, trade on 5 to suppress a contaminating strike).
    Existing configs that don't set this key behave identically.
  - Trader config preference: vev_v1.5-moduleA-only.json,
    vev_v1.5-scalp-only.json, vev_v1.json, inlined CONFIG. Hard Rules
    #10/#11 still hold: no `os` module references anywhere.
  - Lambda log gains `mB` (flag value) and `b_tgt` (Module-B target
    dict); both useful for verifying the flag is honored at runtime.

Inherits from v1.1-v1.5: min_hold/cooldown gating, passive-first
...
CONFIG = {
    ...
    "delta_hedge_band": 30,
    "delta_hedge_agg_band": 60,
    "delta_hedge_throttle_ticks": 100,
    "delta_hedge_max_step": 30,
    "vev_position_limit": 200,
    "exit_mode": "legacy",
    "hold_ticks": 200,
    "enable_spread_gate": False,
    "spread_gate_max_ticks": 2,
}
...
try:
    ...
    # Canonical: vev_v1.5-moduleA-only.json (overwritten by the sweep
    # driver each combo; copied from -baseline.json for a baseline run).
    for _name in ("vev_v1.5-moduleA-only.json",
                  "vev_v1.5-scalp-only.json",
                  "vev_v1.json"):
        _p = _here / _name
        if _p.is_file():
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ---- Per-strike inputs --------------------------------------------
        # active_strikes: strikes we may emit orders on.
        # smile_fit_strikes: strikes whose market IVs anchor the per-tick
        # quadratic smile. Decoupled from active_strikes so we can fit a
        # wider/different universe than we trade (e.g. fit on all 6 OTM
        # vouchers, trade only 5). Defaults to active_strikes for
        # back-compat with all configs that don't specify it.
        active_strikes: List[int] = cfg["active_strikes"]
        fit_strikes: List[int] = cfg.get("smile_fit_strikes", active_strikes)
        mids_universe = sorted(set(active_strikes) | set(fit_strikes))
        voucher_depths: Dict[int, OrderDepth] = {}
        voucher_mids: Dict[int, float] = {}
        for K in mids_universe:
        ...
        # Only fit_strikes contribute to the smile fit; moneyness is
        # computed for every strike in voucher_mids so residuals can be
        # evaluated at active strikes whether or not they're in the fit.
        moneyness_per_strike: Dict[int, float] = {}
        fit_iv_inputs: Dict[int, float] = {}
        ...
        if len(fit_iv_inputs) >= min_strikes:
            ms = [moneyness_per_strike[K] for K in fit_iv_inputs]
            ivs = [fit_iv_inputs[K] for K in fit_iv_inputs]
        ...
        # Iterate only the strikes we actually trade; fit-only strikes
        # contribute to the smile but don't need residual buffers.
        ...
        # spread_gate: when enabled, suppresses Module-A entry/add/flip
        # signals when the per-strike touch spread (best_ask − best_bid)
        # exceeds spread_gate_max_ticks. Closing branches are unaffected.
        # Hypothesis (P3.9b): wide-spread |z|>2 events are adverse-
        # selection bait — counterparties only lift our wide quote when
        # they have edge. Narrowing entries to tight-spread regimes
        # should improve closed-RT.
        spread_gate_enabled = bool(cfg.get("enable_spread_gate", False))
        spread_gate_max = int(cfg.get("spread_gate_max_ticks", 2))
        spread_per_K: Dict[int, int] = {}
        for K in active_strikes:
            d = voucher_depths.get(K)
            if d is None:
                continue
            bb, ba = best_bid_ask(d)
            if bb is None or ba is None:
                continue
            spread_per_K[K] = int(ba - bb)
        spread_blocked_count = 0

        # exit_mode: "legacy" | "loose_z" | "time_based" | "hybrid".
        # legacy / loose_z: close when |z| < z_close (z_close acts as the
        #                   single knob; "loose_z" is just a config-name
        #                   convention for z_close > legacy default).
        # time_based: ignore z for closing; close after hold_ticks ticks.
        # hybrid: close on whichever of {|z|<z_close, hold expired} fires first.
        exit_mode = str(cfg.get("exit_mode", "legacy"))
        hold_ticks_cfg = int(cfg.get("hold_ticks", 200))
        open_ticks_state: Dict[str, int] = saved.get("open_ticks", {})
        close_trig_this_tick: Dict[str, str] = {}
        ...
        for K in active_strikes:
            ...
            if z < -z_open:
                if spread_gate_enabled and spread_per_K.get(K, 10**9) > spread_gate_max:
                    desired = prev
                    spread_blocked_count += 1
                else:
                    desired = cap
            elif z > z_open:
                if spread_gate_enabled and spread_per_K.get(K, 10**9) > spread_gate_max:
                ...
                z_trigger = abs(z) < z_close
                time_trigger = False
                if exit_mode in ("time_based", "hybrid") and prev != 0:
                    ot = open_ticks_state.get(str(K))
                    if ot is not None and (tick_count - ot) >= hold_ticks_cfg:
                        time_trigger = True
                if exit_mode == "time_based":
                    if time_trigger:
                        desired = 0
                        close_trig_this_tick[str(K)] = "t"
                    else:
                        desired = prev
                    ...
                    if z_trigger:
                        desired = 0
                        if prev != 0:
                            close_trig_this_tick[str(K)] = "z"
                    elif time_trigger:
                        desired = 0
                        close_trig_this_tick[str(K)] = "t"
                    else:
                        desired = prev
                else:  # legacy or loose_z
                    if z_trigger:
                        desired = 0
                        if prev != 0:
                            close_trig_this_tick[str(K)] = "z"
            ...
            if desired_dir != prev_dir:
                if prev_dir == 0:
                    if ticks_since < cooldown:
                        ...
                        close_trig_this_tick.pop(str(K), None)
                else:
            ...
            # Track open-tick per strike. Reset on fresh open from flat
            # OR on an outright sign flip (new exposure starts now).
            if prev_dir == 0 and desired_dir != 0:
                open_ticks_state[str(K)] = tick_count
            elif desired_dir == 0:
                open_ticks_state.pop(str(K), None)
            elif desired_dir != prev_dir and prev_dir != 0:
                open_ticks_state[str(K)] = tick_count
            module_a_targets[K] = desired
        saved["module_a_meta"] = a_meta
        saved["module_a_pos"] = {str(K): v for K, v in module_a_targets.items()}
        saved["open_ticks"] = open_ticks_state

        # ---- Module B: base-IV mean reversion (feature-flagged) -----------
        enable_b = bool(cfg.get("enable_module_b", False))
        ...
        if enable_b:
            ...
            # Defensive: if a previous tick (or stale traderData blob)
            # populated module_b_pos with non-zero entries, force them to
            # 0 here AND assert that the targets we'll merge into the
            # combined book are all zero. Crash loudly rather than emit
            # silent Module-B orders.
            saved["module_b_pos"] = {"5200": 0, "5300": 0}
            assert all(v == 0 for v in module_b_targets.values()), (
                "enable_module_b=False but module_b_targets non-zero: "
                f"{module_b_targets}"
            )
        ...
        # aggressive_z_threshold may be JSON Infinity, the string "inf",
        # or a finite number. Infinity disables aggression entirely.
        _az_raw = cfg.get("aggressive_z_threshold", 2.5)
        if isinstance(_az_raw, str) and _az_raw.lower() in ("inf", "infinity"):
            aggressive_z = float("inf")
        else:
            aggressive_z = float(_az_raw)
        ...
        # ---- Module D: VEV delta hedge (feature-flagged) ------------------
        # Computes portfolio delta from CURRENT voucher positions (not
        # targets) and hedges with VEV. Band-based + throttled to avoid
        # recreating Module-C churn:
        #   - hedge only when |delta| > delta_hedge_band
        #   - at most one hedge action per delta_hedge_throttle_ticks
        #   - passive (post limit at our touch) by default
        #   - cross spread only when |delta| > delta_hedge_agg_band
        # When the flag is off, this block is a no-op and VEV stays
        # untraded (matches the old Module-C-stripped behavior).
        enable_dh = bool(cfg.get("enable_delta_hedge", False))
        portfolio_delta_now = 0.0
        hedge_qty_signed = 0
        hedge_action = 0
        hedge_passive = 0
        if enable_dh:
            for _K in ALL_VOUCHER_STRIKES:
                _sym = voucher_symbol(_K)
                _pos = state.position.get(_sym, 0)
                if _pos == 0:
                    continue
                _m = math.log(_K / S) / math.sqrt(T)
                _iv = smile_iv(smile_coefs, _m)
                _, _delta_K, _ = bs_call(S, _K, T, _iv)
                portfolio_delta_now += _pos * _delta_K
            vev_cur_pos = state.position.get(PRODUCT_VEV, 0)
            portfolio_delta_now += vev_cur_pos  # VEV is delta-1

            dh_band = float(cfg.get("delta_hedge_band", 30))
            dh_agg_band = float(cfg.get("delta_hedge_agg_band", 60))
            dh_throttle = int(cfg.get("delta_hedge_throttle_ticks", 100))
            dh_max_step = int(cfg.get("delta_hedge_max_step", 30))
            vev_lim = int(cfg.get("vev_position_limit", 200))

            last_hedge_tick = int(saved.get("last_hedge_tick", -10**9))
            ticks_since_hedge = tick_count - last_hedge_tick

            if abs(portfolio_delta_now) > dh_band and ticks_since_hedge >= dh_throttle:
                target_hedge = -int(round(portfolio_delta_now))
                if target_hedge > dh_max_step:
                    target_hedge = dh_max_step
                elif target_hedge < -dh_max_step:
                    target_hedge = -dh_max_step
                new_vev_pos = vev_cur_pos + target_hedge
                if new_vev_pos > vev_lim:
                    target_hedge = vev_lim - vev_cur_pos
                elif new_vev_pos < -vev_lim:
                    target_hedge = -vev_lim - vev_cur_pos

                if target_hedge != 0 and vev_depth is not None:
                    aggressive = abs(portfolio_delta_now) > dh_agg_band
                    vev_target_pos = vev_cur_pos + target_hedge
                    vev_orders = emit_passive_or_marketable(
                        PRODUCT_VEV, vev_depth, vev_cur_pos, vev_target_pos,
                        escalate=aggressive,
                    )
                    if vev_orders:
                        result[PRODUCT_VEV] = vev_orders
                        hedge_action = 1
                        hedge_passive = 0 if aggressive else 1
                        hedge_qty_signed = target_hedge
                        saved["last_hedge_tick"] = tick_count
        ...
        try:
            print(json.dumps({
                ...
                "z_c": round(z_c_val, 3),
                "mB": int(enable_b),
                "b_tgt": {str(K): module_b_targets[K] for K in module_b_targets},
                "pas": pas_strikes,
                "agg": agg_strikes,
                ...
                "b_pnl": round(pnl_b, 1),
                "mD": int(enable_dh),
                "pf_d": round(portfolio_delta_now, 2) if enable_dh else 0,
                "hdg_q": hedge_qty_signed,
                "hdg_a": hedge_action,
                "hdg_p": hedge_passive,
                "exm": exit_mode[:3],
                "cls": close_trig_this_tick,
                "sg": int(spread_gate_enabled),
                "spr": spread_per_K,
                "sg_blk": spread_blocked_count,
            }))
        except Exception:
            pass
