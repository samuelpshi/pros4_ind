"""
Round 1 Trader — v9-r1
=======================

KELP-analog market making for ACO + Config-A drift capture with circuit breaker for IPR.

Strategy sources:
  ACO: PLAN.md §ACO — KELP-style filtered-mmbot-mid + reversion-beta fair value,
       take/clear/make execution stack, slow-oscillation range bias, deflection kill switch.
  IPR: PLAN.md §IPR — Config A (buy-and-hold to limit=80), passive-bid entry refinement,
       tighter skim (offset=1, size=8), long-only hard guard, realized-drift circuit breaker.

All parameters are named constants in the two blocks below. Every parameter
has a comment pointing to the analysis section that justified its value.
"""
