...
Source: r1_eda_summary.md §Section 4 Open Questions Status. 7 still-open questions after Pass 2 EDA.

| # | Question | Bucket | Resolution / Assumption |
|---|----------|--------|------------------------|
| ACO-1 | Is a ConversionObservation populated for ACO? | **Validate in live submission 1** | No conversion for ACO is described in any available rules text (r1_product_mechanics.md §ACO Fee/Tariff). If a non-null ConversionObservation appears in the first live submission's TradingState, inspect it and assess whether it represents an arbitrage or hedge mechanism. |
| ACO-3 | Are there bot-specific quoting rules for ACO? | **Unresolvable from current data — assumption + risk** | Assumption: bots quote symmetrically, no single bot has a named tell. The adverse-volume filter (volume >= 15) is our best proxy for distinguishing market-maker bots from retail noise. Risk: if a large bot has a non-symmetric quoting rule that the adverse-volume filter fails to capture, our mmbot-mid estimate degrades. |
| ACO-5 | Is PnL marked to mid or to IMC's internal fair value? | **Answered by this plan** | Mark-to-mid. Backtest.py uses mid-price marking explicitly. IMC's internal marking may differ, but this is not controllable from our side. Accept the potential discrepancy and use the local backtest as primary. |
| IPR-2 | Does IPR carry position across days, or reset? | **Unresolvable from current data — assumption + risk** | Assumption: position resets to 0 at each day boundary. Evidence: the v9 trader re-buys 80 units at each day start without checking prior position — if it works in the IMC backtest, position resets. Risk: if position carries, we re-buy 80 when already long 80, causing all buy orders to be rejected by the position limit enforcer. This would silently fail without error (orders just drop). Validate in live submission 1 by checking whether we hold 80 or 0 at the start of day -1. |
| IPR-3 | Is a ConversionObservation populated for IPR? | **Validate in live submission 1** | Same as ACO-1. No conversion described in rules. Flag for inspection on first live observation. |
| IPR-5 | Does the drift ever reverse (harvest/maturity event)? | **Unresolvable from current data — assumption + risk** | Assumption: drift continues deterministically at +1001.3/day for the duration of Round 1. Evidence: 3 days of uniform positive drift (std=1.8). Risk: the drift has never reversed in training data, so we have no empirical estimate of reversal probability. The circuit breaker (§IPR (d)) is the primary risk mitigation. If the circuit breaker triggers on the live submission day, it is a strong signal that the drift thesis has failed. |
| IPR-6 | Can you go short IPR, and is there borrowing cost? | **Answered by this plan** | Position mechanics allow short IPR (limit is -80 to +80). But shorting IPR with a deterministic upward drift is structurally irrational under the current thesis (ipr_mm_synthesis.md §The critical caveat). The long-only floor (§IPR (c)) hard-codes this policy. IPR-6 is closed as "will not pursue under current thesis; revisit only if IPR-5 confirms reversal." |

**Triage summary: 2 Answered by this plan / 2 Validate in live submission 1 / 3 Unresolvable from current data.**
